    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (queue.size == 0) {
            const embed = new EmbedBuilder()
                ...
                .setColor('Yellow')
                .setDescription(this.language.queue.responses.noMusic)
                .setTimestamp()
        ...
        const embed = new EmbedBuilder()
            ...
            .setTimestamp()
            .setFooter({
                text: this.t(this.language.queue.responses.pages, `1/${pages.length}`)
            })

        const nextPage = new ButtonBuilder()
            .setEmoji('⏩')
            .setLabel(this.language.queue.responses.nextPage)
            .setStyle(ButtonStyle.Primary)
            .setCustomId('1')

        const previousPage = new ButtonBuilder()
            .setEmoji('⏪')
            .setLabel(this.language.queue.responses.prevPage)
            .setStyle(ButtonStyle.Primary)
            .setCustomId('2')
        ...
        const message = await interaction.editReply({
            embeds: [embed], components: pages.length > 1 ? [row] : undefined
        })
        ...
        collector.on('collect', async (i) => {
            ...
            embed.setDescription(pages[page].map((track, i) => `${((page + 1) * 10) - 10 + i + 1}º - ${track.title}`).join('\n'))
                .setFooter({
                    text: this.t(this.language.queue.responses.pages, `${page + 1}/${pages.length}`)
                })
            ...
            .on('end', async (collected, reason) => {
                ...
                await interaction.editReply({
                    embeds: [embed], components: [row], content: this.language.queue.responses.noTime
                })
            })
